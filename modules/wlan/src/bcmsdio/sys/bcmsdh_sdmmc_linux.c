...
static int dhd_suspend (void)
{
	...
	if ( NULL != dhd_pub_global ) {
		...
		do {
    		...
    		/* dhdsdio_bussleep_hack(dhd_pub_global,1); */
...
#if 0
	if(wake_lock_active(&wlan_host_wakelock) != 0)
	{
		/* If there is a pending wakelock. Destroy it*/
		printk("[JJ] Destroying wake lock \n"); //TODO-JJ
...
#if 0
	if(gpio == GPIO_LEVEL_HIGH)
	{
		printk(KERN_INFO "[WIFI] Holding Wake Lock for 5 secs \n");  // TODO-JJ	
		wake_lock_timeout(&wlan_host_wakelock, 5*HZ);
	}
#endif

	/*
	 * WL_HOST_WAKE is configured as Active High output signal.
...
int dhd_register_hwakeup(void)
{
	...
	//GPIO config already done in mach-instinctq.c
