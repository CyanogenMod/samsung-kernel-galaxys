 ...
 *
 * $Id: bcmsdh_linux.c,v 1.42.10.10.2.9 2009/10/29 00:00:32 Exp $
 */
...
//	set_irq_wake(sdhcinfo->oob_irq, 1);
	...
	error = request_irq(sdhcinfo->oob_irq, wlan_oob_irq, IRQF_TRIGGER_RISING,
		"bcmsdh_sdmmc", NULL);

	if (error) {
		SDLX_MSG(("bcmsdh_register_oob_intr: request_irq() failed. err=%d\n", error));
		return -ENODEV;
	}
