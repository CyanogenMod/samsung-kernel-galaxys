...
dhd_bus_rxctl(struct dhd_bus *bus, uchar *msg, uint msglen)
{
	...
	if (rxlen) {
		...
		uint8 devpend = bcmsdh_cfg_read(bus->sdh, SDIO_FUNC_0, SDIOD_CCCR_INTPEND, NULL);
		DHD_ERROR(("%s: resumed on timeout. INTPEND=0x%x\n", __FUNCTION__, devpend));
	...
	if (bus->poll && (++bus->polltick >= bus->pollrate)) {
		...
		if (!bus->intr || (bus->intrcount == bus->lastintrs)) {
			...
			if (intstatus) {
				...
				bus->dpc_sched = TRUE;
				DHD_INFO(("dhd_bus_watchdog: trigger dhd_dpc()\n"));
				dhd_sched_dpc(bus->dhd);
