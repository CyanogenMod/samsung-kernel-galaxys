...
static int s3c6400_ac97_probe(struct platform_device *pdev, struct snd_soc_dai *dai) //trb
{
	int ret;
        unsigned int gpio;
...
static void s3c6400_ac97_remove(struct platform_device *pdev, struct snd_soc_dai *dai) //trb
{
	s3cdbg("Entered %s\n", __FUNCTION__);
                                  ...
                                  struct snd_soc_dai *dai)
{
	...
	//struct snd_soc_dai *cpu_dai = rtd->dai->cpu_dai; //trb
	
	s3cdbg("Entered %s\n", __FUNCTION__);

	if (substream->stream == SNDRV_PCM_STREAM_PLAYBACK)
		rtd->dai->cpu_dai->dma_data = &s3c6400_ac97_pcm_stereo_out; //trb
	else
#ifdef CONFIG_SOUND_S5PC100_WM9713_INPUT_STREAM_MIC
		rtd->dai->cpu_dai->dma_data = &s3c6400_ac97_mic_mono_in; //trb
#else /* Input Stream is LINE-IN */
		rtd->dai->cpu_dai->dma_data = &s3c6400_ac97_pcm_stereo_in; //trb
#endif
...
static int s3c6400_ac97_hifi_prepare(struct snd_pcm_substream *substream, struct snd_soc_dai *dai)
{
...
static int __init s5pc1xx_ac97_init(void)
{
        return snd_soc_register_dais(s3c6400_ac97_dai,
                                     ARRAY_SIZE(s3c6400_ac97_dai));
}
module_init(s5pc1xx_ac97_init);

static void __exit s5pc1xx_ac97_exit(void)
{
        snd_soc_unregister_dais(s3c6400_ac97_dai,
                                ARRAY_SIZE(s3c6400_ac97_dai));
}
module_exit(s5pc1xx_ac97_exit);
