 ...
 *    23rd Jun 2009   Modifications as per v 2.6.29 
 */
...
static int __init smdkc100_init(void)
{
	...
	val = __raw_readl(S5PC1XX_GPCPUD);
	val &= ~((3<<0) | (3<<2) | (3<<4) | (3<<6) | (3<<8));
	val |= (2<<0) | (2<<2) | (2<<4) | (2<<6) | (2<<8);
	__raw_writel(val, S5PC1XX_GPCPUD);

	val = __raw_readl(S5PC1XX_GPCCON);
	val &= ~((0xf<<0) | (0xf<<4) | (0xf<<8) | (0xf<<12) | (0xf<<16));
	val |= (4<<0) | (4<<4) | (4<<8) | (4<<12) |  (4<<16);
	__raw_writel(val, S5PC1XX_GPCCON);
