...
/* SMDK6442 has 12MHZ Osc attached to WM8580 */
#define SMDK6442_WM8580_OSC_FREQ (12000000)
...
static int smdk6442_play_opt;
static int smdk6442_rec_opt;
...
static int smdk6442_hw_params(struct snd_pcm_substream *substream,
	struct snd_pcm_hw_params *params)
{
	...
	ret = snd_soc_dai_set_pll(codec_dai, WM8580_PLLA, SMDK6442_WM8580_OSC_FREQ, pll_out);
#else
 ...
 */
static struct snd_soc_ops smdk6442_ops = {
	.hw_params = smdk6442_hw_params,
};

static void smdk6442_ext_control(struct snd_soc_codec *codec)
{
	/* set up jack connection */
	if(smdk6442_play_opt == PLAY_51){
		snd_soc_dapm_enable_pin(codec, "Front-L/R");
		snd_soc_dapm_enable_pin(codec, "Center/Sub");
		snd_soc_dapm_enable_pin(codec, "Rear-L/R");
	}else if(smdk6442_play_opt == PLAY_STEREO){
		snd_soc_dapm_enable_pin(codec, "Front-L/R");
	...
	if(smdk6442_rec_opt == REC_MIC){
		snd_soc_dapm_enable_pin(codec, "MicIn");
		snd_soc_dapm_disable_pin(codec, "LineIn");
	}else if(smdk6442_rec_opt == REC_LINE){
...
static int smdk6442_get_pt(struct snd_kcontrol *kcontrol,
	struct snd_ctl_elem_value *ucontrol)
{
	ucontrol->value.integer.value[0] = smdk6442_play_opt;
	return 0;
}

static int smdk6442_set_pt(struct snd_kcontrol *kcontrol,
	struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_codec *codec = snd_kcontrol_chip(kcontrol);

	if(smdk6442_play_opt == ucontrol->value.integer.value[0])
		return 0;

	smdk6442_play_opt = ucontrol->value.integer.value[0];
	smdk6442_ext_control(codec);
	return 1;
}

static int smdk6442_get_cs(struct snd_kcontrol *kcontrol,
	struct snd_ctl_elem_value *ucontrol)
{
	ucontrol->value.integer.value[0] = smdk6442_rec_opt;
	return 0;
}

static int smdk6442_set_cs(struct snd_kcontrol *kcontrol,
	struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_codec *codec =  snd_kcontrol_chip(kcontrol);

	if(smdk6442_rec_opt == ucontrol->value.integer.value[0])
		return 0;

	smdk6442_rec_opt = ucontrol->value.integer.value[0];
	smdk6442_ext_control(codec);
	return 1;
}

/* smdk6442 card dapm widgets */
...
static const struct soc_enum smdk6442_enum[] = {
	SOC_ENUM_SINGLE_EXT(3, play_function),
	SOC_ENUM_SINGLE_EXT(3, rec_function),
};

static const struct snd_kcontrol_new wm8580_smdk6442_controls[] = {
	SOC_ENUM_EXT("Playback Target", smdk6442_enum[0], smdk6442_get_pt,
		smdk6442_set_pt),
	SOC_ENUM_EXT("Capture Source", smdk6442_enum[1], smdk6442_get_cs,
		smdk6442_set_cs),
};

static int smdk6442_wm8580_init(struct snd_soc_codec *codec)
{
	int i, err;

	/* Add smdk6442 specific controls */
	for (i = 0; i < ARRAY_SIZE(wm8580_smdk6442_controls); i++) {
		err = snd_ctl_add(codec->card,
			snd_soc_cnew(&wm8580_smdk6442_controls[i], codec, NULL));
		if (err < 0)
			return err;
	}

	/* Add smdk6442 specific widgets */
	snd_soc_dapm_new_controls(codec, wm8580_dapm_widgets,
				  ARRAY_SIZE(wm8580_dapm_widgets));

	/* Set up smdk6442 specific audio path audio_map */
	...
	/* Setup Default Route */
	smdk6442_play_opt = PLAY_STEREO;
	smdk6442_rec_opt = REC_LINE;
	smdk6442_ext_control(codec);

	return 0;
}

static struct snd_soc_dai_link smdk6442_dai[] = {
{
	.name = "WM8580",
	.stream_name = "WM8580 HiFi Playback",
	.cpu_dai = &s3c_i2s_pdat.i2s_dai,
	.codec_dai = &wm8580_dai[WM8580_DAI_PAIFRX],
	.init = smdk6442_wm8580_init,
	.ops = &smdk6442_ops,
},
};

static struct snd_soc_card smdk6442 = {
	.name = "smdk6442",
	.lp_mode = 0,
	.platform = &s3c_pcm_pdat.pcm_pltfm,
	.dai_link = smdk6442_dai,
	.num_links = ARRAY_SIZE(smdk6442_dai),
};

static struct wm8580_setup_data smdk6442_wm8580_setup = {
	.i2c_address = 0x1b,
};

static struct snd_soc_device smdk6442_snd_devdata = {
	.card = &smdk6442,
	.codec_dev = &soc_codec_dev_wm8580,
	.codec_data = &smdk6442_wm8580_setup,
};

static struct platform_device *smdk6442_snd_device;

static int __init smdk6442_audio_init(void)
{
	int ret;
	...
	smdk6442_snd_device = platform_device_alloc("soc-audio", 0);
	if (!smdk6442_snd_device)
		return -ENOMEM;

	platform_set_drvdata(smdk6442_snd_device, &smdk6442_snd_devdata);
	smdk6442_snd_devdata.dev = &smdk6442_snd_device->dev;
	ret = platform_device_add(smdk6442_snd_device);

	if (ret)
		platform_device_put(smdk6442_snd_device);
...
static void __exit smdk6442_audio_exit(void)
{
	platform_device_unregister(smdk6442_snd_device);
}

module_init(smdk6442_audio_init);
module_exit(smdk6442_audio_exit);

module_param (lowpower, int, 0444);

/* Module information */
MODULE_DESCRIPTION("ALSA SoC SMDK6442 WM8580");
MODULE_LICENSE("GPL");
