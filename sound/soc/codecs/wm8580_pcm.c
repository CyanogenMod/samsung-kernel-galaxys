...
static unsigned int aifb_glob,aifa_glob;
...
static int wm8580_get_mic_path(struct snd_kcontrol *kcontrol,
        struct snd_ctl_elem_value *ucontrol)
{
//
//
...
static int wm8580_set_mic_path(struct snd_kcontrol *kcontrol,
        struct snd_ctl_elem_value *ucontrol)
{
 return 0;
}
...
SOC_ENUM_EXT("MIC Path", path_control_enum[0],
                wm8580_get_mic_path, wm8580_set_mic_path),
		...
		int pll_id, unsigned int freq_in, unsigned int freq_out)
{
	...
	debug_msg("...check point .entered into %s..pll_id=%d..\n",__func__,pll_id);
				 ...
				 struct snd_soc_dai *dai)
{
	...
	{
	debug_msg("^^^^^Check point hw_params_playback_setting_for_pcm..@=%d..\n",__LINE__);
        wm8580_write(codec,WM8580_CLKSEL,0x01C);
        wm8580_write(codec,WM8580_SAIF1,3<<6);
        wm8580_write(codec,WM8580_PWRDN1,0x03a);
        wm8580_write(codec,WM8580_PWRDN2,0x038);
        wm8580_write(codec,WM8580_SAIF2,aifa_glob);
        wm8580_write(codec,WM8580_PAIF3,aifb_glob);
        wm8580_write(codec,WM8580_GPO3,0x1FF);
        wm8580_write(codec,WM8580_DAC_CONTROL2,0x005);
	}
	else
        {
        debug_msg("^^^^^Check point hw_params_capture_settingi_for_pcm..@=%d..\n",__LINE__);
        wm8580_write(codec,WM8580_PWRDN1, 0x07c);
        wm8580_write(codec,WM8580_PWRDN2, 0x03f);
        wm8580_write(codec,WM8580_CLKSEL, 0x01c);
        wm8580_write(codec,WM8580_SAIF1, 0x0c0);
        wm8580_write(codec,WM8580_ADC_CONTROL1, 0x000);
        wm8580_write(codec,WM8580_SAIF2,aifa_glob); 
        wm8580_write(codec,WM8580_PAIF4,aifb_glob);
        }

	debug_msg("..check point in %s...wm8580_paif3=0x%x..wm8580_saif2=0x%x..\n",__func__,wm8580_read(codec,WM8580_PAIF3),wm8580_read(codec,WM8580_SAIF2));
        debug_msg("..check point..WM8580_CLKSEL=0x%x..\n",wm8580_read(codec,WM8580_CLKSEL));
        debug_msg("..check point..WM8580_SAIF1=0x%x..\n",wm8580_read(codec,WM8580_SAIF1));
        debug_msg("..check point..WM8580_DAC_CONTROL1=0x%x..\n",wm8580_read(codec,WM8580_DAC_CONTROL1));
        debug_msg("..check point..WM8580_DAC_CONTROL2=0x%x..\n",wm8580_read(codec,WM8580_DAC_CONTROL2));
        debug_msg("..check point..WM8580_DAC_CONTROL5=0x%x..\n",wm8580_read(codec,WM8580_DAC_CONTROL5));
        debug_msg("..check point..WM8580_PWRDN1=0x%x..\n",wm8580_read(codec,WM8580_PWRDN1));
        debug_msg("..check point..WM8580_PWRDN2=0x%x..\n",wm8580_read(codec,WM8580_PWRDN2));
        debug_msg("..check point.. WM8580_PLLB4=0x%x..\n",wm8580_read(codec, WM8580_PLLB4));
				      ...
				      unsigned int fmt)
{
	...
	aifb_glob  = wm8580_read(codec, WM8580_PAIF3 );
	aifa_glob  = wm8580_read(codec, WM8580_SAIF2 );

	aifb_glob &= ~(WM8580_AIF_FMT_MASK | WM8580_AIF_LRP | WM8580_AIF_BCP | 0x0C | 0x180);
	aifa_glob &= ~(WM8580_AIF_FMT_MASK | WM8580_AIF_LRP | WM8580_AIF_BCP | 0x0C);


	switch (fmt & SND_SOC_DAIFMT_FORMAT_MASK) {
	case SND_SOC_DAIFMT_I2S:
		can_invert_lrclk = 1;
		aifb_glob |= WM8580_AIF_FMT_I2S;
		break;
	case SND_SOC_DAIFMT_RIGHT_J:
		can_invert_lrclk = 1;
		aifb_glob |= WM8580_AIF_FMT_RIGHTJ;
		break;
	case SND_SOC_DAIFMT_LEFT_J:
		can_invert_lrclk = 1;
		aifb_glob |= WM8580_AIF_FMT_LEFTJ;
		break;
	case SND_SOC_DAIFMT_DSP_A:
		can_invert_lrclk = 0;
		aifb_glob |= WM8580_AIF_FMT_DSP;
		break;
	case SND_SOC_DAIFMT_DSP_B:
		can_invert_lrclk = 0;
		aifb_glob |= WM8580_AIF_FMT_DSP;
		aifa_glob |= WM8580_AIF_FMT_DSP;
		aifb_glob |= WM8580_AIF_LRP;
		aifa_glob |= WM8580_AIF_LRP;
		break;
	default:
		return -EINVAL;
	}
	...
	case SND_SOC_DAIFMT_IB_IF:
		if (!can_invert_lrclk)
			return -EINVAL;
		aifb_glob |= WM8580_AIF_BCP;
		aifb_glob |= WM8580_AIF_LRP;
		break;

	case SND_SOC_DAIFMT_IB_NF:
		aifb_glob |= WM8580_AIF_BCP;
		aifa_glob |= WM8580_AIF_BCP;
		break;
	...
	aifb_glob |= 0x100;
	aifa_glob |= (1<<7|1<<6);
	debug_msg("..check point in %s...aifb_glob=0x%x..wm8580_saif2=0x%x..\n",__func__,aifb_glob,wm8580_read(codec,WM8580_SAIF2));
...
static int wm8580_init(struct snd_soc_device *socdev)
{
	...
	codec->num_dai = 1;//ARRAY_SIZE(wm8580_dai);
...
static int __init wm8580_modinit(void)
{
	...
	return snd_soc_register_dai(&wm8580_dai);
}
...
static void __exit wm8580_exit(void)
{
	...
	snd_soc_unregister_dai(&wm8580_dai);
}
