...
static int __init
s5pc110_leds_init(void)
{
	if (machine_is_smdkc110())
		leds_event = s5pc110_leds_event;
	else
		return -1;

	if (machine_is_smdkc110())
	{
               gpio_request(S5PC11X_GPH1(4), "GPH1");
               gpio_direction_output(S5PC11X_GPH1(4), 1);
               if(gpio_get_value(S5PC11X_GPH1(4)) == 0)
               {
                      printk(KERN_WARNING "LED: can't set GPH1(4) to output mode\n");
               }

               gpio_request(S5PC11X_GPH1(5), "GPH1");
               gpio_direction_output(S5PC11X_GPH1(5), 1);
               if(gpio_get_value(S5PC11X_GPH1(5)) == 0)
               {
                      printk(KERN_WARNING "LED: can't set GPH1(5) to output mode\n");
               }

               gpio_request(S5PC11X_GPH1(6), "GPH1");
               gpio_direction_output(S5PC11X_GPH1(6), 1);
               if(gpio_get_value(S5PC11X_GPH1(6)) == 0)
               {
                      printk(KERN_WARNING "LED: can't set GPH1(6) to output mode\n");
               }

               gpio_request(S5PC11X_GPH1(7), "GPH1");
               gpio_direction_output(S5PC11X_GPH1(7), 1);
               if(gpio_get_value(S5PC11X_GPH1(7)) == 0)
               {
