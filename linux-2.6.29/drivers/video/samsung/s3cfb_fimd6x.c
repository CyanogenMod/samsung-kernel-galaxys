...
int s3cfb_set_clock(struct s3cfb_global *ctrl)
{
...
#if USE_SCLK_FIMD		
#ifdef CONFIG_S3C_FB_MDNIE
	cfg |= (S3C_VIDCON0_CLKSEL_SCLK | S3C_VIDCON0_CLKVALUP_ALWAYS | \
		S3C_VIDCON0_VCLKEN_FREERUN | S3C_VIDCON0_CLKDIR_DIVIDED);
#else
	cfg |= (S3C_VIDCON0_CLKSEL_SCLK | S3C_VIDCON0_CLKVALUP_ALWAYS | \
	...
	div++;  //PKD: increment by one to reduce clock value from 27.7Mhz to 23.8 MHz
...
int s3cfb_set_alpha_blending(struct s3cfb_global *ctrl, int id)
{
	...
	if (id == 0) {
		err("[fb%d] does not support alpha blending\n", id);
		return -EINVAL;
	}
...
int s3cfb_set_chroma_key(struct s3cfb_global *ctrl, int id)
{
	...
	if (id == 0) {
		err("[fb%d] does not support chroma key\n", id);
		return -EINVAL;
	}
        mask_key = (id == 1) ? S3C_WIN1_COLOR_MASK_KEY : \
                   (id == 2) ? S3C_WIN2_COLOR_MASK_KEY : \
                   (id == 3) ? S3C_WIN3_COLOR_MASK_KEY : \
                   S3C_WIN4_COLOR_MASK_KEY;

        cfg = readl(ctrl->regs + S3C_KEYCON(id));
        cfg &= ~(1<<26);
        cfg |= S3C_KEYCON0_KEYBLEN_DISABLE;
        cfg &= ~(1<<24);
        cfg |= S3C_KEYCON0_DIRCON_MATCH_FG;
	// Apply the mask if BPP is 16
	if(ctrl->fb[id]->var.bits_per_pixel == 16)
                cfg |= S3C_KEYCON0_COMPKEY(mask_key);
