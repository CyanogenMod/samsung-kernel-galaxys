...
static int smdkc110_set_pcm_clk(int id)
{
        ...
        case 0:
                ...
                break;
        case 1:
    //writel(((readl(S5P_CLK_SRC6)&~S5P_CLKSRC6_AUDIO1_MASK)|(0x1<<S5P_CLKSRC6_AUDIO1_SHIFT)),S5P_CLK_SRC6);//in case of choosing pcmcdclk
...
//		psr -= 1;
	...
	ret = snd_soc_dai_set_fmt(codec_dai, SND_SOC_DAIFMT_DSP_B |SND_SOC_DAIFMT_IB_NF );
	if (ret < 0)
		return ret;
