...
#include "pwm-S5PC11X.h"

S5PC11X_pwm_chan_t s3c_chans[S3C_PWM_CHANNELS];

static inline void
S5PC11X_pwm_buffdone(S5PC11X_pwm_chan_t *chan, void *dev)
{
...
static int S5PC11X_pwm_start (int channel)
{
	unsigned long tcon;
...
static irqreturn_t S5PC11X_pwm_irq(int irq, void *devpw)
{
	S5PC11X_pwm_chan_t *chan = (S5PC11X_pwm_chan_t *)devpw;
	void *dev=chan->dev;

	/* modify the channel state */
	S5PC11X_pwm_buffdone(chan, dev);

	return IRQ_HANDLED;
}


int S5PC11X_pwm_request(pwmch_t  channel, s3c_pwm_client_t *client, void *dev)
{
	S5PC11X_pwm_chan_t *chan = &s3c_chans[channel];
	unsigned long flags;
	int err;
	...
	if (!chan->irq_claimed) {
		...
		err = request_irq(chan->irq, S5PC11X_pwm_irq, IRQF_DISABLED,
...
int S5PC11X_pwm_free (pwmch_t channel, s3c_pwm_client_t *client)
{
	S5PC11X_pwm_chan_t *chan = &s3c_chans[channel];
	unsigned long flags;
...
int S5PC11X_pwm_set_buffdone_fn(pwmch_t channel, s3c_pwm_cbfn_t rtn)
{
	S5PC11X_pwm_chan_t *chan = &s3c_chans[channel];
...
#define S5PC11X_pwm_suspend NULL
#define S5PC11X_pwm_resume  NULL

struct sysdev_class pwm_sysclass = {
	.name           = "s3c-pwm",	
	.suspend	= S5PC11X_pwm_suspend,
	.resume		= S5PC11X_pwm_resume,
};


/* initialisation code */

static int __init S5PC11X_init_pwm(void)
{
	S5PC11X_pwm_chan_t *cp;
	int channel;
	int ret;
	...
	for (channel = 0; channel < S3C_PWM_CHANNELS; channel++) {
		...
		memset(cp, 0, sizeof(S5PC11X_pwm_chan_t));
